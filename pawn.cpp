...
Pawn::Pawn(PlayerType player, QObject *parent)
    ...
    imagePath = (player == PlayerType::white) ? QString("C:/Programming/ChessProject/gfx/white_pawn.png") :
                    QString("C:/Programming/ChessProject/gfx/black_pawn.png");
}
    ...
    if (pos.size() == 1)
    {
        ...
        if (pos[0].y != 0)
        {
            ...
            if (IfPosCorrect(new_pos))
            {
                ...
                else if (cells[new_pos.x][new_pos.y].white_enPassant && owner == PlayerType::black) pos.push_back(new_pos);
                else if (cells[new_pos.x][new_pos.y].black_enPassant && owner == PlayerType::white) pos.push_back(new_pos);
            }
